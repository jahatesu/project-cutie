...
// Enhanced airplane animation with smooth movement
        document.addEventListener('DOMContentLoaded', function() {
            const airplane = document.querySelector('.airplane-container');

            // Add smooth rotation based on path direction
            airplane.style.offsetRotate = 'auto';

            // Add subtle floating effect to hearts
            const hearts = document.querySelectorAll('.floating-heart');
            hearts.forEach((heart, index) => {
                heart.style.animationDelay = `${index * 0.5}s`;
                heart.style.animationDuration = `${3 + index * 0.5}s`;
            });

            // Add interactive hover effects
            const countries = document.querySelectorAll('.country-canada, .country-philippines');
            countries.forEach(country => {
                country.addEventListener('mouseenter', function() {
                    this.style.filter = 'drop-shadow(0 8px 16px rgba(139, 92, 246, 0.3)) brightness(1.1)';
                });
