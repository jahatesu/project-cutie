...
// Airplane animation along the curved path
        ...
        class LoveMazeGame {
            constructor() {
                this.maze = document.getElementById('mazeGrid');
                ...
                this.cellSize = 20;
                this.cols = Math.floor(460 / this.cellSize);
                this.rows = Math.floor(390 / this.cellSize);

                this.playerPos = { x: 1, y: 1 };
                this.finishPos = { x: this.cols - 2, y: this.rows - 2 };
                this.stepCount = 0;
                this.startTime = Date.now();
                this.gameWon = false;

                this.walls = [];
                this.createHeartMaze();
            ...
            createHeartMaze() {
                // Create heart-shaped maze walls
                const heartWalls = [
                    // Outer heart shape
                    ...this.createHeartBorder(),
                    // Inner maze paths
                    ...this.createMazePaths()
                ];

                heartWalls.forEach(wall => {
                    const wallEl = document.createElement('div');
                    wallEl.className = 'maze-wall';
                    wallEl.style.left = wall.x * this.cellSize + 'px';
                    wallEl.style.top = wall.y * this.cellSize + 'px';
                    wallEl.style.width = wall.width * this.cellSize + 'px';
                    wallEl.style.height = wall.height * this.cellSize + 'px';
            ...
            createHeartBorder() {
                ...
                // Create heart shape border
                for (let x = 0; x < this.cols; x++) {
                    for (let y = 0; y < this.rows; y++) {
                        if (!this.isInsideHeart(x, y, centerX, centerY)) {
            ...
            isInsideHeart(x, y, centerX, centerY) {
                const dx = (x - centerX) / (this.cols * 0.3);
                const dy = (y - centerY) / (this.rows * 0.3);
            ...
            createMazePaths() {
                ...
                const mazePattern = [
                    ...
                    { x: 8, y: 5, width: 3, height: 1 },
                    { x: 2, y: 7, width: 5, height: 1 },
                    { x: 9, y: 9, width: 4, height: 1 },
                    { x: 4, y: 11, width: 6, height: 1 },
                    { x: 6, y: 13, width: 3, height: 1 },

                    // Vertical walls
                    { x: 5, y: 2, width: 1, height: 3 },
                    { x: 9, y: 4, width: 1, height: 4 },
                    { x: 3, y: 8, width: 1, height: 3 },
                    { x: 11, y: 6, width: 1, height: 5 },
                    { x: 7, y: 10, width: 1, height: 3 },
                    { x: 13, y: 8, width: 1, height: 4 },

                    // Additional maze complexity
                    { x: 6, y: 6, width: 2, height: 1 },
                    { x: 10, y: 12, width: 1, height: 2 },
                    { x: 4, y: 14, width: 3, height: 1 },
                    { x: 12, y: 3, width: 1, height: 3 },
                ];

                mazePattern.forEach(pattern => {
                    if (this.isValidWallPosition(pattern)) {
                        walls.push(pattern);
                    }
                });
            ...
            isValidWallPosition(wall) {
                // Check if wall position is within heart bounds and doesn't block start/finish
                const startX = this.playerPos.x;
                const startY = this.playerPos.y;
                const finishX = this.finishPos.x;
                const finishY = this.finishPos.y;
                ...
                for (let x = wall.x; x < wall.x + wall.width; x++) {
                    for (let y = wall.y; y < wall.y + wall.height; y++) {
                        ...
                        if (!this.isInsideHeart(x, y, Math.floor(this.cols / 2), Math.floor(this.rows / 2))) {
                            return false;
                        }
                    }
                }
            ...
            canMoveTo(x, y) {
                ...
                // Check if position is inside heart
                if (!this.isInsideHeart(x, y, Math.floor(this.cols / 2), Math.floor(this.rows / 2))) {
                    return false;
                }
            ...
            updateFinishPosition() {
                this.finish.style.left = this.finishPos.x * this.cellSize - 5 + 'px';
                this.finish.style.top = this.finishPos.y * this.cellSize - 5 + 'px';
            }
            ...
            checkWin() {
                if (this.playerPos.x === this.finishPos.x && this.playerPos.y === this.finishPos.y) {
                    this.gameWon = true;
