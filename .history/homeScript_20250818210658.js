...
let stars = document.querySelectorAll('#stars span'); // get individual stars

window.addEventListener('scroll', () => {
  let value = window.scrollY;

  // move each star individually
  stars.forEach((star, i) => {
    let speed = 0.1 + (i % 5) * 0.05; // give stars slightly different speeds
    star.style.top = `translateX(${value * speed}px)`;
  });
